...
// Simplified Hybrid Parameters - more greedy focused
struct HybridParams {
    int num_ants = 6;
    ...
    double beta = 2.0;
    double rho = 0.1;
    double greedy_bias = 0.98;  // Even more greedy
    int elite_solutions = 2;
    double local_search_prob = 0.8;
    int restart_threshold = 8;
};
...
private:
    ...
    int select_aco_pickup(const set<int> &candidates) {
        ...
        if (total_prob == 0.0) return candidate_list[0];
