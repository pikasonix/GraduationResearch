...
Instance parse_input(const string &filename) {
    ...
    if (i + size > lines.size()) {
        cout << "Error: Not enough node data in input file. Expected " << size << " nodes" << endl;
        exit(1);
    }
    ...
    if (i >= lines.size()) {
        cout << "Error: EDGES section not found in input file" << endl;
        exit(1);
    }
    ...
    if (i + size > lines.size()) {
        cout << "Error: Not enough edge data in input file. Expected " << size << " rows" << endl;
        exit(1);
    }
    ...
    for (int j = 0; j < size; j++) {
        ...
        if (row.size() < size) {
            cout << "Error: Invalid edge data at row " << j << ". Expected " << size << " columns, got " << row.size() << endl;
            exit(1);
        }
    ...
    cout << "  Route time: " << inst.route_time << endl;
...
void solve(const string &input_file, const string &output_file, int num_routes) {
    try {
        cout << "Starting to parse input file..." << endl;
        ...
        if (pickup_nodes.empty()) {
            cout << "Warning: No pickup nodes found in the instance" << endl;
            // Create an empty solution
            HybridSolution empty_solution(num_routes);
            write_output(output_file, empty_solution);
            cout << "Empty solution written to " << output_file << endl;
            return;
        }

        int required_pairs = pickup_nodes.size();
        cout << "Required pairs: " << required_pairs << endl;
        ...
        // Create an empty solution in case of error
        HybridSolution error_solution(num_routes);
        write_output(output_file, error_solution);
    } catch (...) {
        cout << "Unknown error during solving" << endl;
        // Create an empty solution in case of error
        HybridSolution error_solution(num_routes);
        write_output(output_file, error_solution);
    }
...
int main(int argc, char *argv[]) {
    try {
        ...
        // Check if we have command line arguments for files
        if (argc >= 3) {
            input_file = argv[1];
            output_file = argv[2];

            // If we have more than 3 arguments, parse algorithm parameters
            if (argc >= 4) {
                num_routes = atoi(argv[3]);
                if (argc >= 5)
                    hybrid_params.num_ants = atoi(argv[4]);
                if (argc >= 6)
                    hybrid_params.max_iterations = atoi(argv[5]);
                if (argc >= 7)
                    hybrid_params.alpha = atof(argv[6]);
                if (argc >= 8)
                    hybrid_params.beta = atof(argv[7]);
                if (argc >= 9)
                    hybrid_params.rho = atof(argv[8]);
                if (argc >= 10)
                    hybrid_params.tau_max = atof(argv[9]);
                if (argc >= 11)
                    hybrid_params.tau_min = atof(argv[10]);
                if (argc >= 12)
                    hybrid_params.greedy_bias = atof(argv[11]);
                if (argc >= 13)
                    hybrid_params.elite_solutions = atoi(argv[12]);
                if (argc >= 14)
                    hybrid_params.local_search_prob = atof(argv[13]);
                if (argc >= 15)
                    hybrid_params.restart_threshold = atoi(argv[14]);

                cout << "Parameters loaded from command line:" << endl;
                cout << "  Routes: " << num_routes << endl;
                cout << "  Ants: " << hybrid_params.num_ants << endl;
                cout << "  Iterations: " << hybrid_params.max_iterations << endl;
                cout << "  Alpha: " << hybrid_params.alpha << endl;
                ...
                // Parse parameters from first line of input file (legacy mode)
                num_routes = parse_parameters_from_input(input_file);
            }
        } else {
            // Parse parameters from first line of input file (legacy mode)
            num_routes = parse_parameters_from_input(input_file);
