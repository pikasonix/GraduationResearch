...
impl SINTEFSolutionBuilder {
    ...
    pub fn build(self) -> SINTEFSolution {
        ...
        SINTEFSolution {
            ...
            date_string: self.date_string.unwrap_or(current_date),
            routes: self.routes,
        }
    }
}
...
pub fn write_sintef_solution(
    ...
    // Authors:          Carlo Sartori and Luciana Buriol
    writeln!(file, "Authors:          {}", solution.authors)?;
    // Date:             2019-2-11
    writeln!(file, "Date:             {}", solution.date_string)?;
    // Reference:        A matheuristic approach to the PDPTW (to be submitted).
    writeln!(file, "Reference:        {}", solution.reference)?;
    // Solution
