...
<#
.SYNOPSIS
    Automated benchmark runner for all Li & Lim instances
    
.DESCRIPTION
    Runs all instances with appropriate time limits:
...
# Configuration
$groups = @(
    @{Name = "pdptw_100";  TimeLimit = 60;   Pattern = "resources\instances\lilim\pdptw_100\*.txt";  ExpectedCount = 6}
    @{Name = "pdptw_200";  TimeLimit = 120;  Pattern = "resources\instances\lilim\pdptw_200\*.txt";  ExpectedCount = 6}
    @{Name = "pdptw_400";  TimeLimit = 600;  Pattern = "resources\instances\lilim\pdptw_400\*.txt";  ExpectedCount = 6}
    @{Name = "pdptw_1000"; TimeLimit = 1200; Pattern = "resources\instances\lilim\pdptw_1000\*.txt"; ExpectedCount = 3}
)
...
Write-Host " AUTOMATED BENCHMARK - ALL LI & LIM INSTANCES" -ForegroundColor $C_HEADER
...
Write-Host "  - pdptw_400:  600s  (10 min) x 6 instances = ~60 min" -ForegroundColor $C_INFO
Write-Host "  - pdptw_1000: 1200s (20 min) x 3 instances = ~60 min" -ForegroundColor $C_INFO
Write-Host "  Total estimated time: ~138 minutes (2.3 hours)" -ForegroundColor $C_WARNING
...
foreach ($group in $groups) {
    ...
    Write-Host "`n[$groupIndex/4] ========== $groupName (${timeLimit}s) ==========" -ForegroundColor $C_HEADER
